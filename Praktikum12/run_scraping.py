...
def setup_dask_client():
    """Menginisialisasi Dask Client dan terhubung ke Scheduler Docker."""
    try:
        # KOREKSI: Gunakan nama service 'dask-scheduler' dan tingkatkan timeout menjadi 60 detik
        client = Client("tcp://dask-scheduler:8786", timeout="60s") 
        print("✅ Berhasil tersambung ke Dask Cluster yang sudah ada.")
        
        # Cek jumlah Worker yang terhubung ke Scheduler
        num_workers = len(client.scheduler_info()['workers'])
        print(f"Jumlah Worker di Cluster Utama: {num_workers}")
        
        if num_workers == 0:
            raise Exception("Cluster terhubung, tetapi tidak ada Worker yang terdaftar.")
            
        return client
    except Exception as e:
        # Menangkap semua exception (termasuk TimeoutError)
        print(f"⚠️ Gagal tersambung ke Scheduler Dask di tcp://dask-scheduler:8786. Error: {e}")
        # Hentikan eksekusi jika gagal terhubung ke Cluster utama
        print("❌ Eksekusi dibatalkan: Pastikan dask-scheduler sudah berjalan dengan 'docker compose up -d' dan Worker terhubung.")
        exit(1) 
...
def run_distributed_scraping():
    ...
    num_workers = len(client.scheduler_info()['workers']) # Ambil jumlah worker yang benar
    
    print(f"\nMemulai scraping untuk {total_urls} URL...")
    ...
    # Jumlah partisi diatur menjadi 2x jumlah thread total
